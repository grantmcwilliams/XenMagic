...
class properties:
    ...
    def fill_listcustomfields(self):
       clist = []
       pool_ref = self.all_pools.keys()[0] 
    ...
    def fill_listhomeserver(self, ref):
        list = []
        path = 0
        i = 0
        for host in self.all_hosts.keys():
            ...
            if self.all_hosts[host]['enabled']:
                if host == ref:
                    path = i 
                list.append([host, "images/tree_connected_16.png", self.all_hosts[host]['name_label'],
                                hostmemory, 
                                ])
            i = i + 1
        return list, path

    def fill_vdi_location(self, ref):
        list = []
        i = 0
        selected = 0
