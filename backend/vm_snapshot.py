...
class vmsnapshot:
    ...
    def delete_snapshot(self, ref, ref_vm):
        for vbd in self.all_vms[ref]['VBDs']:
            if self.all_vbd[vbd]['VDI'] != "OpaqueRef:NULL":
                res = self.connection.Async.VDI.destroy(self.session_uuid, self.all_vbd[vbd]['VDI'])
                if "Value" in res:
                    ...
                    print res
        for vbd in self.all_vms[ref]['VBDs']:
                res = self.connection.Async.VBD.destroy(self.session_uuid, vbd)
                if "Value" in res:
                    self.track_tasks[res['Value']] = ref_vm
                    self.track_tasks[res['Value']] = ref
                else:
                    print res
        res = self.connection.Async.VM.destroy(self.session_uuid, ref)
        if "Value" in res:
            self.track_tasks[res['Value']] = ref_vm
            self.track_tasks[res['Value']] = ref
            return "OK"
    ...
    def create_template_from_snap(self, ref, name):
        ...
        if "Value" in res:
            self.track_tasks[res['Value']] = ref
            self.convert_to_template.append(res['Value'])
        else:
            print res
